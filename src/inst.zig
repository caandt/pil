...
pub const Inst = union(enum) {
    ...
    Addi: packed struct { op: u7 = 3 << 1, rd: u3, rs1: u3, i: u19 },
    Subi: packed struct { op: u7 = 4 << 1, rd: u3, rs1: u3, i: u19 },
    ...
    Sftl: packed struct { op: u7 = 18 << 1, rd: u3, rs: u3, i: u5, _pad: u14 = 0 },
    Sftr: packed struct { op: u7 = 19 << 1, rd: u3, rs: u3, i: u5, _pad: u14 = 0 },
        ...
        return switch ((n >> 1) % (1 << 5)) {
            inline 1...std.meta.fields(Inst).len - 1 => |op| {
                inline for (std.meta.fields(Inst)) |inst| {
                    if (std.meta.fields(inst.type)[0].defaultValue().? == op << 1) {
                        return @unionInit(Inst, inst.name, @bitCast(n));
                    }
                }
            },
            else => null,
